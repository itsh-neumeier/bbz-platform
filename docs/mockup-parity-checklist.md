...
| 5 | generic API client — command envelope, `X-Command-Id`, 409, correlation id echo | `lib/apiClient.ts` | [#99](../../issues/99) (E07-04) | done — client + `ApiError`/`ConflictError`/`AuthExpiredError`, caught in 4 components; `e2e/api-conflict.spec.ts` proves the 409 path against a real two-tab race |
...
| 16 | i18n — DE locale complete + missing-key lint | `src/i18n/de.json`, `scripts/i18n-lint.mjs` | [#119](../../issues/119) (E07-14) | done — `i18n:lint` (381 keys, all resolve) blocks the `frontend` CI job; `t()`/`d()` used i18n-first throughout; date/time via vue-i18n's `d()` (ADR-0017). Its own AC calls for unit + lint only, no Playwright leg |
...
| 20 | comms sidebar scaffold — tabs phone / call / phonebook / history | `CommsSidebar` tabs, `stores/calls.ts`, `lib/telephony.ts` | [#127](../../issues/127) (E07-18) | done — the four tabs, keypad + line picker, waiting-call queue, active-call controls + documentation, mini phone-book, call history, line strip; `CommsSidebar.spec.ts`, `e2e/comms-sidebar.spec.ts` (keyboard operability + tab-switch state persistence) |
...
| 23 | contact priority blue / orange / red, call-priority animation | Epic 14 · #299, #301 | done — contact priority (#299): `ContactPriorityBadge.vue` = text label on the §13.9 colour (niedrig=blau/mittel=orange/hoch=rot), AA contrast, colour is a secondary cue, consistent across phone-book / quick-dial / mini-search. Call-queue (#301): sorted high→low, graded pulse (`wq-pulse-high` 1.4s / `wq-pulse-medium` 2.6s / low none), explicit `@media (prefers-reduced-motion)`, unknown caller shows its number. `ContactPriorityBadge.spec.ts`, `a11y.spec.ts`, `e2e/call-queue.spec.ts` |
...
| 26 | quick-dial dialog ("Kurzwahl öffnen" overlay) | Epic 11 · #225 | done — a "Kurzwahl öffnen" button in the Telefon tab opens `QuickDialOverlay.vue` (native `<dialog>`, no permanent grid in the layout), lists `quick_dial`-flagged contacts, choosing one dials via the existing `dialContact()` path; `QuickDialOverlay.spec.ts`, `e2e/quick-dial.spec.ts` |
| 27 | contact ↔ call-history link (UI) | Epic 14 · #303 | done — phone-book contact detail gains a "Rufhistorie" fieldset (recent calls + "letzter Kontakt"), `calls.view_history`-gated; the comms sidebar's Historie tab links a resolved caller to `/telefonbuch?contact=<id>`; `e2e/contact-history.spec.ts` |
