...
#: the dead PJSIP context a WebRTC endpoint points at (ADR-0035) — the browser
#: never dials the dialplan; BBZ originates to it over ARI.
SIP_WEBRTC_CONTEXT = "bbz-webrtc-inert"
...
class SipWebrtcEndpoint(Base, TimestampMixin):
    """A per-operator WebRTC SIP endpoint on BBZ's Asterisk (ADR-0035).

    The browser registers to ``transport-wss`` with these credentials; BBZ
    bridges the operator's WebRTC channel to the trunk channel over ARI. The
    SIP password is a secret: only ``auth_password_ciphertext`` (Fernet,
    ``BBZ_SIP_ENCRYPTION_KEY`` via :mod:`bbz_core.infra.sip_secrets`), never
    returned by the admin API, logged, or audited. It is disclosed once, to the
    operator's own session, by ``GET /api/v1/telephony/webrtc-credentials``.
    """

    __tablename__ = "sip_webrtc_endpoints"
    __table_args__ = (UniqueConstraint("auth_username", name="uq_sip_webrtc_auth_username"),)

    #: the operator this endpoint belongs to (one per person)
    user_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("users.id", ondelete="CASCADE"), primary_key=True
    )
    #: the SIP username, generated by BBZ (``op-<8 hex>``) — unique, never chosen
    auth_username: Mapped[str] = mapped_column(String(64), server_default=text("''"))
    #: Fernet ciphertext of the SIP password — ``""`` = unset
    auth_password_ciphertext: Mapped[str] = mapped_column(Text, server_default=text("''"))
    #: master switch per operator
    enabled: Mapped[bool] = mapped_column(Boolean, server_default=text("true"))
    updated_by: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("users.id", ondelete="SET NULL")
    )
